...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.jodesk.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.jodesks.update", [$jodesk->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label class="required" for="nama_jodesk">{{ trans('cruds.jodesk.fields.nama_jodesk') }}</label>
                <input class="form-control {{ $errors->has('nama_jodesk') ? 'is-invalid' : '' }}" type="text" name="nama_jodesk" id="nama_jodesk" value="{{ old('nama_jodesk', $jodesk->nama_jodesk) }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('deskripsi') ? 'is-invalid' : '' }}" type="text" name="deskripsi" id="deskripsi" value="{{ old('deskripsi', $jodesk->deskripsi) }}" required>
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('status') ? 'is-invalid' : '' }}" name="status" id="status">
                    ...
                    @foreach(App\Jodesk::STATUS_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('status', $jodesk->status) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('petugas') ? 'is-invalid' : '' }}" name="petugas[]" id="petugas" multiple>
                    @foreach($petugas as $id => $petugas)
                        <option value="{{ $id }}" {{ (in_array($id, old('petugas', [])) || $jodesk->petugas->contains($id)) ? 'selected' : '' }}>{{ $petugas }}</option>
                    @endforeach
                </select>
