...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.berkasMasuk.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.berkas-masuks.store") }}" enctype="multipart/form-data">
            @csrf
            <div class="form-group">
                <label class="required" for="tgl_masuk">{{ trans('cruds.berkasMasuk.fields.tgl_masuk') }}</label>
                <input class="form-control datetime {{ $errors->has('tgl_masuk') ? 'is-invalid' : '' }}" type="text" name="tgl_masuk" id="tgl_masuk" value="{{ old('tgl_masuk') }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('petugas_loket') ? 'is-invalid' : '' }}" type="text" name="petugas_loket" id="petugas_loket" value="{{ old('petugas_loket', '') }}" required>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('penerima_masuk') ? 'is-invalid' : '' }}" name="penerima_masuk_id" id="penerima_masuk_id" required>
                    @foreach($penerima_masuks as $id => $penerima_masuk)
                        <option value="{{ $id }}" {{ old('penerima_masuk_id') == $id ? 'selected' : '' }}>{{ $penerima_masuk }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('no_berkas') ? 'is-invalid' : '' }}" type="text" name="no_berkas" id="no_berkas" value="{{ old('no_berkas', '') }}" required>
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('jenis_kegiatan') ? 'is-invalid' : '' }}" name="jenis_kegiatan" id="jenis_kegiatan" required>
                    ...
                    @foreach(App\BerkasMasuk::JENIS_KEGIATAN_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('jenis_kegiatan', '0') === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('no_surattugas') ? 'is-invalid' : '' }}" type="text" name="no_surattugas" id="no_surattugas" value="{{ old('no_surattugas', '') }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('tgl_surattugas') ? 'is-invalid' : '' }}" type="text" name="tgl_surattugas" id="tgl_surattugas" value="{{ old('tgl_surattugas') }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('no_su') ? 'is-invalid' : '' }}" type="text" name="no_su" id="no_su" value="{{ old('no_su', '') }}">
                @if($errors->has('no_su'))
                    <div class="invalid-feedback">
                        {{ $errors->first('no_su') }}
                    </div>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('no_hak') ? 'is-invalid' : '' }}" type="text" name="no_hak" id="no_hak" value="{{ old('no_hak', '') }}">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('isiberkas') ? 'is-invalid' : '' }}" name="isiberkas[]" id="isiberkas" multiple required>
                    @foreach($isiberkas as $id => $isiberkas)
                        <option value="{{ $id }}" {{ in_array($id, old('isiberkas', [])) ? 'selected' : '' }}>{{ $isiberkas }}</option>
                    @endforeach
                </select>
