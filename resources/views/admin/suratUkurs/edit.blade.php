...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.suratUkur.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.surat-ukurs.update", [$suratUkur->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('id_kecamatan') ? 'is-invalid' : '' }}" name="id_kecamatan_id" id="id_kecamatan_id" required>
                    @foreach($id_kecamatans as $id => $id_kecamatan)
                        <option value="{{ $id }}" {{ (old('id_kecamatan_id') ? old('id_kecamatan_id') : $suratUkur->id_kecamatan->id ?? '') == $id ? 'selected' : '' }}>{{ $id_kecamatan }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('id_desa') ? 'is-invalid' : '' }}" name="id_desa_id" id="id_desa_id" required>
                    @foreach($id_desas as $id => $id_desa)
                        <option value="{{ $id }}" {{ (old('id_desa_id') ? old('id_desa_id') : $suratUkur->id_desa->id ?? '') == $id ? 'selected' : '' }}>{{ $id_desa }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('no_su') ? 'is-invalid' : '' }}" type="text" name="no_su" id="no_su" value="{{ old('no_su', $suratUkur->no_su) }}" required>
                @if($errors->has('no_su'))
                    <div class="invalid-feedback">
                        {{ $errors->first('no_su') }}
                    </div>
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('tahun') ? 'is-invalid' : '' }}" type="text" name="tahun" id="tahun" value="{{ old('tahun', $suratUkur->tahun) }}" required>
                @if($errors->has('tahun'))
                    <div class="invalid-feedback">
                        {{ $errors->first('tahun') }}
                    </div>
