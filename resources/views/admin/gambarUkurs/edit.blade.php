...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.gambarUkur.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.gambar-ukurs.update", [$gambarUkur->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('id_pengukuran') ? 'is-invalid' : '' }}" name="id_pengukuran_id" id="id_pengukuran_id" required>
                    @foreach($id_pengukurans as $id => $id_pengukuran)
                        <option value="{{ $id }}" {{ (old('id_pengukuran_id') ? old('id_pengukuran_id') : $gambarUkur->id_pengukuran->id ?? '') == $id ? 'selected' : '' }}>{{ $id_pengukuran }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('id_kecamatan') ? 'is-invalid' : '' }}" name="id_kecamatan_id" id="id_kecamatan_id" required>
                    @foreach($id_kecamatans as $id => $id_kecamatan)
                        <option value="{{ $id }}" {{ (old('id_kecamatan_id') ? old('id_kecamatan_id') : $gambarUkur->id_kecamatan->id ?? '') == $id ? 'selected' : '' }}>{{ $id_kecamatan }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('id_desa') ? 'is-invalid' : '' }}" name="id_desa_id" id="id_desa_id" required>
                    @foreach($id_desas as $id => $id_desa)
                        <option value="{{ $id }}" {{ (old('id_desa_id') ? old('id_desa_id') : $gambarUkur->id_desa->id ?? '') == $id ? 'selected' : '' }}>{{ $id_desa }}</option>
                    @endforeach
                </select>
