    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.pengukurans.update", [$pengukuran->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('id_berkas') ? 'is-invalid' : '' }}" name="id_berkas_id" id="id_berkas_id" required>
                    @foreach($id_berkas as $id => $id_berkas)
                        <option value="{{ $id }}" {{ (old('id_berkas_id') ? old('id_berkas_id') : $pengukuran->id_berkas->id ?? '') == $id ? 'selected' : '' }}>{{ $id_berkas }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('pembantu_ukurs') ? 'is-invalid' : '' }}" name="pembantu_ukurs[]" id="pembantu_ukurs" multiple required>
                    @foreach($pembantu_ukurs as $id => $pembantu_ukur)
                        <option value="{{ $id }}" {{ (in_array($id, old('pembantu_ukurs', [])) || $pengukuran->pembantu_ukurs->contains($id)) ? 'selected' : '' }}>{{ $pembantu_ukur }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('desa') ? 'is-invalid' : '' }}" name="desa_id" id="desa_id" required>
                    @foreach($desas as $id => $desa)
                        <option value="{{ $id }}" {{ (old('desa_id') ? old('desa_id') : $pengukuran->desa->id ?? '') == $id ? 'selected' : '' }}>{{ $desa }}</option>
                    @endforeach
                </select>
                @if($errors->has('desa'))
                    <div class="invalid-feedback">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('kecamatan') ? 'is-invalid' : '' }}" name="kecamatan_id" id="kecamatan_id" required>
                    @foreach($kecamatans as $id => $kecamatan)
                        <option value="{{ $id }}" {{ (old('kecamatan_id') ? old('kecamatan_id') : $pengukuran->kecamatan->id ?? '') == $id ? 'selected' : '' }}>{{ $kecamatan }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('no_gu') ? 'is-invalid' : '' }}" type="text" name="no_gu" id="no_gu" value="{{ old('no_gu', $pengukuran->no_gu) }}" required>
                @if($errors->has('no_gu'))
                    <div class="invalid-feedback">
                        {{ $errors->first('no_gu') }}
                    </div>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('no_su_baru') ? 'is-invalid' : '' }}" type="text" name="no_su_baru" id="no_su_baru" value="{{ old('no_su_baru', $pengukuran->no_su_baru) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control datetime {{ $errors->has('tanggal_pengukuran') ? 'is-invalid' : '' }}" type="text" name="tanggal_pengukuran" id="tanggal_pengukuran" value="{{ old('tanggal_pengukuran', $pengukuran->tanggal_pengukuran) }}" required>
            ...
            <div class="form-group">
                ...
                <textarea class="form-control ckeditor {{ $errors->has('keterangan') ? 'is-invalid' : '' }}" name="keterangan" id="keterangan">{!! old('keterangan', $pengukuran->keterangan) !!}</textarea>
