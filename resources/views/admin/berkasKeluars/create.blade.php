...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.berkasKeluar.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.berkas-keluars.store") }}" enctype="multipart/form-data">
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('berkasmasuk') ? 'is-invalid' : '' }}" name="berkasmasuk_id" id="berkasmasuk_id" required>
                    @foreach($berkasmasuks as $id => $berkasmasuk)
                        <option value="{{ $id }}" {{ old('berkasmasuk_id') == $id ? 'selected' : '' }}>{{ $berkasmasuk }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control datetime {{ $errors->has('tgl_keluar') ? 'is-invalid' : '' }}" type="text" name="tgl_keluar" id="tgl_keluar" value="{{ old('tgl_keluar') }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('penerima_keluar') ? 'is-invalid' : '' }}" type="text" name="penerima_keluar" id="penerima_keluar" value="{{ old('penerima_keluar', '') }}" required>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('pemberi_keluar') ? 'is-invalid' : '' }}" name="pemberi_keluar_id" id="pemberi_keluar_id" required>
                    @foreach($pemberi_keluars as $id => $pemberi_keluar)
                        <option value="{{ $id }}" {{ old('pemberi_keluar_id') == $id ? 'selected' : '' }}>{{ $pemberi_keluar }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('pengukuran') ? 'is-invalid' : '' }}" name="pengukuran_id" id="pengukuran_id">
                    @foreach($pengukurans as $id => $pengukuran)
                        <option value="{{ $id }}" {{ old('pengukuran_id') == $id ? 'selected' : '' }}>{{ $pengukuran }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('isiberkas_keluars') ? 'is-invalid' : '' }}" name="isiberkas_keluars[]" id="isiberkas_keluars" multiple required>
                    @foreach($isiberkas_keluars as $id => $isiberkas_keluar)
                        <option value="{{ $id }}" {{ in_array($id, old('isiberkas_keluars', [])) ? 'selected' : '' }}>{{ $isiberkas_keluar }}</option>
                    @endforeach
                </select>
